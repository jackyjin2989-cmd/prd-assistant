...
2. **事实分层**：区分已确认事实、产品判断、假设（待确认）和开放问题，不把推断写成现状。
3. **最小追问**：只追问会改变核心流程、角色权限、资金/合规、数据归属、不可逆操作或验收结论的问题。
...
- URL：仅当用户要求观察现状，或关键结论必须通过页面核实时，调用 `browser-observer`。
...
- 用户明确要求访问或核实现有网站
- 用户提供 URL，且 PRD 的关键产品事实无法从现有材料确认
...
无法判断且会改变核心功能时，保留冲突及其影响并请求确认；不影响核心结论时，采用最小假设并明确标注。
...
- 最终文档不用删除线保留被淘汰内容；若确需变更记录，放在用户明确要求的变更日志中。
