...
- 用户明确要求访问、核实、抓取、截图、提取或比较授权页面；
- 某项事实无法从现有材料确认，并且该事实会改变核心方案或确定性验收，同时用户已提供可访问的授权页面。

URL、截图模糊或裁切、缺少普通交互状态、PC/H5 差异等均不单独触发观察。材料缺口不影响核心方案或验收时继续处理并标为待确认；没有授权页面时集中追问，不访问网站。
