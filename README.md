...
脚本检查三个 Skill 的目录与 frontmatter、必需参考文件、仓库内非图片 Markdown 链接（图片目标未检查）、基础敏感文本模式，以及 `prd-assistant` 的默认内容边界和观察/原型低成本分流规则。它还检查图片指南的职责边界与关键规则，确认 `html-prototype` 承接原型实现与验证且自动截图仅在用户明确要求时执行，检测 `responsive-guide.md` 和 `visual-validation.md` 不含强制双端和固定矩阵残留，确认写法指南含成功判定规则，并核对运行版与仓库版文件一致性。脚本不解析 Markdown 图片目标、不判断图片视觉内容或实际远程状态，也不替代人工语义审查，这些仍需在具体交付中人工复核。
