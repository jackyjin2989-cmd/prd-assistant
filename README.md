...
### 维护约定：同一文件的改动要串行，改完回读确认

同一个文件的一批修改**不要并行下发**：并行编辑是「各自读原文 → 各自写回」，后写的那次会覆盖前一次的改动，而两处都会报告成功。本次就踩到过 —— 两条针对同一文件的修改，最终只有一条落盘。

所以：

1. 同一文件的多个改动，一次只发一处，等前一处落盘再发下一处；
2. 改完用 `grep -n` 回读关键行确认（**macOS 自带 grep 是 BSD 版，不支持 `\|` 交替，要用 `grep -E`**，否则会误判成「没有匹配」）；
